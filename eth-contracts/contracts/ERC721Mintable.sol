...
contract Ownable {
    ...
    //  3) create an 'onlyOwner' modifier that throws if called by any account other than the owner.
     modifier onlyOwner(){
        require(isOwner(),'Caller should be the owner');
        _;
    }
    ...
    function _transferOwnership(address newOwner) internal {
        require(newOwner != address(0), "Address should be a valid one");
...
contract ERC721 is Pausable, ERC165 {
    ...
    function balanceOf(address owner) public view returns (uint256) {
        ...
        require(owner != address(0), "Token owner addres is not valid");
        return _ownedTokensCount[owner].current();
    }

    function ownerOf(uint256 tokenId) public view returns (address) {
        // TODO return the owner of the given tokenId
        address currentTokenOwner = _tokenOwner[tokenId];
        require(currentTokenOwner != address(0), "Token owner addres is not valid");
        return currentTokenOwner;
...
//    @dev Approves another address to transfer the given token ID
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(ownerOf(tokenId) == from,"Address provided should be the owner of the token");
        // TODO: require token is being transfered to valid address
        require(to != address(0),"Address to transfer should be valid ");
        // TODO: clear approval
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function _setTokenURI(uint256 tokenId) internal{
        require(_exists(tokenId),'Token id does not exist');
        _tokenURIs[tokenId] = strConcat(_baseTokenURI,usingOraclize.uint2str(tokenId));
    }
